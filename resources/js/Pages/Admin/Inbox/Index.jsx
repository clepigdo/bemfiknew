        ...
        <div className="min-h-screen bg-slate-900 text-white p-6 md:p-10 font-sans">
            ...
            <div className="mb-10">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h1 className="text-3xl font-black tracking-tight">Pesan Masuk</h1>
                </div>
                <p className="text-slate-400">Daftar aspirasi dan pesan kolaborasi dari pengunjung website.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
                ...
                <div className="flex gap-4">
                    <div className="px-4 py-2 bg-slate-800 rounded-xl border border-slate-700 text-xs font-bold">
                        TOTAL: <span className="text-blue-400">{contacts.length}</span>
                    </div>
                    <div className="px-4 py-2 bg-slate-800 rounded-xl border border-slate-700 text-xs font-bold">
                        BELUM DIBACA: <span className="text-orange-400">{contacts.filter(c => !c.is_read).length}</span>
                    </div>
                </div>
            </div>
                                ...
                                >
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col">
                                            <span className={`font-bold ${!contact.is_read ? 'text-blue-400' : 'text-white'}`}>
                                                {contact.name}
                                            </span>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-2 text-slate-500 whitespace-nowrap">
                                            <Clock size={14} />
                                            {new Date(contact.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        {contact.is_read ? (
