    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white p-6 md:p-10 font-sans">
            ...
            <div className="max-w-3xl mx-auto">
                <Link href={route('programs.index')} className="text-slate-400 hover:text-white flex items-center gap-2 mb-6 text-sm font-bold">
                    <ArrowLeft size={16} /> Kembali
                </Link>

                <div className="bg-slate-800 rounded-3xl border border-white/5 p-8 shadow-2xl">
                    <h1 className="text-2xl font-black mb-8">Buat Program Kerja Baru</h1>
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        {/* Deskripsi */}
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Deskripsi Singkat</label>
                            <textarea 
                                rows="3"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* Tanggal */}
                            <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Tanggal Pelaksanaan</label>
                            ...
                            {/* Status */}
                            <div>
                                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Status Kegiatan</label>
                                <select 
                                    value={data.status}
                        ...
                        {/* Upload Gambar */}
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Poster / Dokumentasi</label>
                            <div className="border-2 border-dashed border-slate-700 rounded-xl p-8 text-center hover:border-blue-500 transition-colors relative">
                                {preview ? (
                                    <img src={preview} alt="Preview" className="h-48 mx-auto rounded-lg object-cover" />
                                ) : (
                        ...
                        <div className="pt-4">
                            ...
                            >
                                {processing ? 'Menyimpan...' : <><Save size={20} /> Simpan Program Kerja</>}
                            </button>
                        </div>
                    </form>
                </div>
